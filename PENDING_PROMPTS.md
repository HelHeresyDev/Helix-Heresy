...
1. Quality-Gated Production Orders and Component Retry Rules
2. Title Screen, Main Menu, and Background Art
3. Prison Foundation and Long-Term Incarceration
4. Prison Breaks, Rescue, and Release
5. Death Row, Appeals, and Execution
6. Reusable Worlds, Random Names, and Run Separation
7. World Themes and Content Boundaries
8. Hex-Based Strategic World Map
...
Design and implement quality requirements for production orders and their sub-orders.

Item quality should be based on both the quality of its components and the quality of final assembly. When the player creates a production order, they should be able to specify minimum quality thresholds for required components. When the system creates or executes sub-orders for that parent order, each component should be checked against the relevant threshold before it is consumed by the final assembly.

If a sub-order produces a component below the specified threshold, that component should not be used for the parent order. It should be placed into storage as a normal physical item with its actual quality, provenance, material history, and any relevant defects. The parent order should then try again until a component meets or exceeds the threshold, the player cancels or changes the order, required resources become unavailable, or the order becomes blocked by a clear production limit.

Questions for discussion:

- Should minimum component quality be specified per component type, as a global order quality target, or both?

  Recommended answer: Start with one overall component quality target on the parent order, then support per-component overrides after the data model is stable.

- What happens to below-threshold components?

  Recommended answer: Store them as ordinary completed items with their actual quality. They can be used for lower-quality orders, sold, recycled, repaired, studied, or manually selected later. Do not delete them or hide them as failed rolls.

- Should retries continue forever?

  Recommended answer: No. Let retries continue while materials and labor are available, but surface warnings after repeated failures and allow the player to pause, lower the threshold, substitute materials, or cancel. The queue should not silently consume all resources without making the failure pattern visible.

- How should final item quality be calculated?

  Recommended answer: Combine component quality, material suitability, tool/workstation condition, operator skill, recipe difficulty, environment, and final assembly quality. Components should matter enough that high-quality assembly cannot fully rescue bad parts, but bad final assembly can still ruin good components.

- Should this apply to chemistry batches and biological products?

  Recommended answer: Start with manufactured components and assembled items. Chemistry batches and living specimens may later use the same quality-gate contract if their quality model fits, but do not force every production type into the first implementation.

Do not modify files until the design has been discussed and the developer explicitly approves implementation.

## 2. Title Screen, Main Menu, and Background Art

Design and implement a real title screen for Helix Heresy.

The game should open to a title screen instead of immediately dropping the player into the management interface. The title screen should include background art, the game title, and clear actions such as New Run, Load Run, Settings, and Credits or About if appropriate.

The title screen should prepare for future world selection, World Theme selection, starting scenario selection, run selection, accessibility settings, and content notices without forcing all of those systems into this pass.

Questions for discussion:

- What should the first title screen include?

  Recommended answer: Include the title, background art, New Run, Load Run or Continue, Settings, and a concise current-build/version note. Keep deeper world and scenario choices behind New Run.

- Should the title screen load before any save state?

  Recommended answer: Yes. Load only the minimal save index and settings needed to populate Continue/Load controls. Do not fully materialize a run until the player chooses one.

- How should background art be handled before final production art exists?

  Recommended answer: Use one approved development background or placeholder illustration with clear semantic asset naming. Do not block title-screen implementation on final art.

- How should accessibility work?

  Recommended answer: Title screen controls must be keyboard-accessible, screen-reader labeled, readable without background art, and compatible with reduced-motion/high-contrast settings.
...
- Can loss of the laboratory or life imprisonment end the run?
  ...
  Recommended answer: No. Those are severe changes in campaign position. The run remains active while the scientist lives.
...
- Which exits should be implemented first?
  ...
  Recommended answer: Support sentence completion, one earned or negotiated early-release route, one physical self-escape plan, and one outside-assisted extraction.
...
- When does an execution become scheduled?
  ...
  Recommended answer: Create a disclosed provisional date at sentencing, then allow jurisdictional review, appeals, stays, and commutation attempts to change it through saved causal decisions.

- What can prevent execution?

  Recommended answer: Successful appeal, sentence commutation, lawful intervention, self-escape, outside rescue, or a physical disruption of the execution process.

- When does the run end?

  Recommended answer: Only when an execution is physically completed and reduces the living scientist to death, or the scientist dies from another cause.
...
Design and implement the strategic world map as a hex grid.

The world map should use hexagonal cells, similar to RimWorld's world map, rather than square tiles. This applies to world-scale geography, biome regions, routes, settlements, city distance, candidate sites, beast territories, political territories, strategic travel, and future campaign expansion.
...
Design and implement the smallest complete, deterministic world that can be generated, named, saved, selected, and viewed before a run begins. Use a finite hex-based strategic map or compatible region graph with land/water boundaries, stable region identities, world scale, diagnostics, and a player-visible preview. Generate a complete low-resolution world before play while leaving exact local maps lazy.
...
Use the existing sprite manifest, loader, atlas workflow, semantic keys, and development sprites to establish and replace assets with a coherent first production-quality set. Preserve footprint anchors, transforms, renderer-neutral semantic keys, DOM glyph fallbacks, accessibility modes, and the approved visual language. Keep this prompt last because world generation and campaign work may introduce new visuals.

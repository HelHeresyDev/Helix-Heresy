(function attachHelixDeathRowCustody(root, factory) {
  ...
  const STATUSES = Object.freeze(["active", "executionProcessDue", "resentencingRequired", "retrialRequired", "releaseOrdered", "commutationTransferRequired", "transferred", "escaped", "released", "deceased"]);
  ...
  function activeStay(value) { return normalizeState(value).stays.find((stay) => ["active", "executionProcessDue", "resentencingRequired", "retrialRequired", "releaseOrdered", "commutationTransferRequired"].includes(stay.status)) || null; }
  ...
  function applyLegalDirective(value, stayId, options = {}, clock = 0) { const state = normalizeState(value); const stay = state.stays.find((entry) => entry.id === cleanId(stayId)); if (!stay || !["active", "executionProcessDue", "resentencingRequired", "retrialRequired", "releaseOrdered", "commutationTransferRequired"].includes(stay.status)) return { state, stay, changed: false }; const at = Math.max(stay.committedAt, finite(clock)); const previous = JSON.stringify({ status: stay.status, calendar: stay.calendar }); if (options.executionAt != null) stay.calendar.provisionalExecutionAt = Math.max(stay.committedAt, finite(options.executionAt)); if (options.stayed) { stay.calendar.executionStatus = "stayed"; if (stay.status === "executionProcessDue") stay.status = "active"; } else if (options.reliefKind) { stay.calendar.executionStatus = "cancelled"; stay.status = options.reliefKind === "resentencing" ? "resentencingRequired" : options.reliefKind === "retrial" ? "retrialRequired" : options.reliefKind === "commutation" ? "commutationTransferRequired" : "releaseOrdered"; } else if (stay.calendar.executionStatus === "stayed") stay.calendar.executionStatus = options.rescheduled ? "rescheduled" : "provisional"; if (options.finalAdverseDecisionAt != null) stay.calendar.finalAdverseDecisionAt = Math.max(stay.committedAt, finite(options.finalAdverseDecisionAt)); const changed = previous !== JSON.stringify({ status: stay.status, calendar: stay.calendar }); if (changed) stay.history.push({ at, action: "capitalLegalDirective", summary: String(options.summary || `Capital legal process set execution to ${stay.calendar.executionStatus} and custody to ${stay.status}.`).trim() }); return { state, stay, changed }; }
  function completeLegalTransfer(value, stayId, destinationKind, clock = 0) { const state = normalizeState(value); const stay = state.stays.find((entry) => entry.id === cleanId(stayId)); if (!stay || !["resentencingRequired", "retrialRequired", "releaseOrdered", "commutationTransferRequired"].includes(stay.status)) return { state, stay, changed: false }; const at = Math.max(stay.committedAt, finite(clock)); stay.status = destinationKind === "release" ? "released" : "transferred"; stay.suppressor.status = "removed"; stay.suppressor.suppressionActive = false; stay.suppressor.removedAt = at; stay.history.push({ at, action: "capitalLegalTransfer", summary: destinationKind === "release" ? "The final appellate mandate physically released the scientist from capital custody." : destinationKind === "finitePrison" ? "Armored corrections transport removed the scientist from capital custody under the executive commutation instrument." : `Armored court transport removed the scientist from capital custody for linked ${destinationKind}.` }); return { state, stay, changed: true }; }
  ...
  function advance(value, clock = 0) {
    ...
    if (stay.calendar.automaticReviewStatus === "scheduled" && stay.calendar.automaticReviewAt <= now) { stay.calendar.automaticReviewStatus = "opened"; stay.history.push({ at: stay.calendar.automaticReviewAt, action: "automaticReviewOpened", summary: "Automatic capital review opened on the linked appellate docket." }); }
  ...
  return { VERSION, HOUR, DAY, PROVISIONAL_EXECUTION_DELAY, AUTOMATIC_REVIEW_DELAY, MINIMUM_POST_DECISION_DELAY, STATUSES, PRIORITIES, CHANNELS, ROUTINE, defaultState, normalizeState, activeStay, commit, setPriority, clearDecision, recordDisciplinaryIncident, applyLegalDirective, completeLegalTransfer, requestCommunication, completeCommunication, advance, nextEvent };
}));

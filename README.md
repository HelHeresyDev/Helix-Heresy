...
- High-severity law-enforcement warrants authorize deterministic named four-officer raids with saved objectives, scope, roles, equipment, communication, force causes, barrier outcomes, seizures, and history. Booking leads into a compact physical temporary jail and opens an evidence-linked criminal case with named court actors, exact charges, public/retained/self representation, privileged counsel conferences, and an escorted secure holographic first appearance. Causal decisions support recognizance, conditions, refundable bail escrow, or continued detention; lawful release removes the jail collar and uses a saved vehicle transfer to the Public Entrance, while strongly supported animancy can justify a separate physical court suppressor. Escape adds charges and a bench warrant without ending the run.
...
- `pretrial-proceedings.js` - Renderer-independent evidence-linked charges, deterministic court actors and counsel, privileged-conference gates, first-appearance scheduling and decisions, release conditions, bail escrow, vehicle-transfer records, and continuing fugitive cases.

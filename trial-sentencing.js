(function attachHelixTrialSentencing(root, factory) {
  ...
  const SENTENCING_SUBMISSIONS = Object.freeze([
    ...
    { id: "penalFlight", label: "Request Penal Flight Instead of Public-Enemy Condemnation", mitigation: 0 },
  ...
  function normalizeOrder(candidate) {
    ...
    return {
      ...
      stayed: Boolean(candidate.stayed), stayOrderId: cleanId(candidate.stayOrderId),
      ...
      penalServiceMonths: Number.isInteger(candidate.penalServiceMonths) && candidate.penalServiceMonths >= 12 && candidate.penalServiceMonths <= 36 ? candidate.penalServiceMonths : null,
      ...
      publicEnemyDesignation: candidate.publicEnemyDesignation && typeof candidate.publicEnemyDesignation === "object" ? { status: candidate.publicEnemyDesignation.status === "entered" ? "entered" : "pending", kind: "publicEnemy", label: String(candidate.publicEnemyDesignation.label || "Public Enemy").trim(), enteredAt: Math.max(0, finite(candidate.publicEnemyDesignation.enteredAt, candidate.issuedAt)), grounds: unique(candidate.publicEnemyDesignation.grounds) } : null, executionMethod: cleanId(candidate.executionMethod),
  ...
  function normalizeCase(candidate, index = 0) {
    ...
    return {
      ...
      actorId: cleanId(source.actorId) || "scientist", sentencingPolicy: source.sentencingPolicy ? JSON.parse(JSON.stringify(source.sentencingPolicy)) : null,
  ...
  function createOrder(state, caseRecord, at, acquittal = false) {
    ...
    if (kind === "deathRow" && caseRecord.strategy.sentencingSubmissionId === "penalFlight" && caseRecord.sentencingPolicy?.penalFlightAvailable) kind = "penalFlight";
    const servicePolicy = caseRecord.sentencingPolicy;
    const serviceAvailable = servicePolicy?.penalLegionAvailable === true && Number.isInteger(servicePolicy.penalServiceMinimumMonths) && Number.isInteger(servicePolicy.penalServiceMaximumMonths) && servicePolicy.penalServiceMinimumMonths >= 12 && servicePolicy.penalServiceMaximumMonths <= 36 && servicePolicy.penalServiceMinimumMonths <= servicePolicy.penalServiceMaximumMonths;
    if (kind === "penalLegion" && !serviceAvailable) { kind = "finitePrison"; reasons.push("No published finite military-service policy supports a penal-legion commitment."); }
    const defs = {
      ...
      penalFlight: { label: "Penal Flight and wilderness banishment", custodial: true, destinationId: "penalFlightDepot", status: caseRecord.custodyStatus === "released" ? "remandPending" : "commitmentPending" },
    ...
    const order = normalizeOrder({ id: `sentencing-order-${state.nextOrderNumber++}`, kind, label: def.label, custodial: def.custodial, issuedAt: at, transferNotBefore: at + (def.custodial ? 4 * HOUR : 30 * 60), destinationId: def.destinationId, status: def.status,
      ...
      penalServiceMonths: kind === "penalLegion" ? Math.max(servicePolicy.penalServiceMinimumMonths, Math.min(servicePolicy.penalServiceMaximumMonths, Math.ceil(exposure))) : null,
      ...
      commitmentId: def.custodial ? `${caseRecord.id}-${kind}-commitment` : "", provisionalExecutionProcessId: kind === "deathRow" ? `${caseRecord.id}-provisional-execution` : "", publicEnemyDesignation: kind === "deathRow" ? { status: "entered", kind: "publicEnemy", label: "Public Enemy", enteredAt: at, grounds: convictions.filter((charge) => capitalEligible({ charges: [charge] })).map((charge) => `${charge.label}: ${charge.verdictReason || "capital-eligible lethal conduct was proven"}`) } : null, executionMethod: kind === "deathRow" ? "publicBeheading" : "", reasons: [...reasons, `${def.label} followed the saved exposure band and jurisdictional eligibility rules.`, ...(kind === "deathRow" ? ["The court entered a separate supported Public Enemy designation and a physical public-beheading method. The death sentence does not kill the scientist; only later medically confirmed lethal physiology can do so."] : [])] });
